...
export async function decrementPromptBalance(email: string) {
  try {
    ...
    if (usersArray.length > 0) {
      ...
      if (firstUser.promptBalance && firstUser.promptBalance > 0) {
        const updatedBalance = firstUser.promptBalance - 1;
        await db
          .update(user)
